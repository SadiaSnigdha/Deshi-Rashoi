...
const placeOrder = async (req, res) => {
  ...
  try {
    ...
    await User.findByPk(req.body.userId).then(user => user.update({ cartData: {} }));
...
const verifyOrder = async (req, res) => {
  ...
  try {
    if (success == "true") {
      await Order.findByPk(orderId).then(order => order.update({ payment: true }));
      res.json({ success: true, message: "Paid" });
    } else {
      await Order.findByPk(orderId).then(order => order.destroy());
...
const updateStatus = async (req, res) => {
  try {
    let userData = await User.findByPk(req.body.userId);
    if (userData && userData.role === "admin") {
      await Order.findByPk(req.body.orderId).then(order => order.update({ status: req.body.status }));
